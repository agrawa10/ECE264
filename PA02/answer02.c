...
void my_strcat(char * s1, const char * s2) {
  ...
  for (i = s1_length; i < s1_length + my_strlen(s2); i++) {
    s1[i] = s2[i - s1_length];
  }
  s1[i] = '\0';
}
...
void my_strncat(char * s1, const char * s2, int num) {
  int i;
  int s1_length = my_strlen(s1);
  for (i = s1_length; i < s1_length + num; i++) {
    s1[i] = s2[i - s1_length];
  }
  s1[i] = '\0';
}
...
const char *my_strstr(const char * s1, const char * s2) {
  ...
  for (i = 0; i < my_strlen(s1); i++) {
    if (s1[i] == s2[0]) {
      j = i + 1;
      k = 1;
      while (s1[j] == s2[k] && k < my_strlen(s2)) {
        j++;
        k++;
      }
      if (k == my_strlen(s2)) {
...
void my_strinsert(char *s1, const char *s2, int pos) {
  int lengthOne = my_strlen(s1);
  int lengthTwo = my_strlen(s2);
  int i;
  int j;
  ...
  if (pos >= lengthOne) {
    ...
    for (i = 1; i <= lengthOne - pos; i++) {
      *(s1 + lengthOne + lengthTwo - i) = *(s1 + lengthOne - i);
    }
    for (j = pos; j < pos + lengthTwo; j++) {
      *(s1 + j) = *(s2 + j - pos);
    }
  *(s1 + lengthOne + lengthTwo) = '\0';
  }
}
...
void my_strdelete(char *s, int pos, int length) {
  if (pos >= my_strlen(s)) {
    ...
    if (my_strlen(s) - pos < length) {
      s[pos] = '\0';
    }
    else {
      int i;
      for (i = pos; i < my_strlen(s) - length; i++) {
      s[i] = s[i + length];
      }
      s[my_strlen(s) - length] = '\0';
    }
  }
}

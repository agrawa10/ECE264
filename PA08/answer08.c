...
SparseNode *SparseNode_create(int index, int value) {
  SparseNode* p = malloc(sizeof(SparseNode));
  p -> index = index;
  p -> value = value;
  p -> left = NULL;
  p -> right = NULL;
...
SparseNode *SparseArray_build(int * indicies, int * values, int length) {
  ...
  SparseNode* p = NULL;
  for (i = 0; i < length; i++) {
    p = SparseArray_insert(p, indicies[i], values[i]);
  }
  return p;
}
...
SparseNode * SparseArray_copy(SparseNode * array) {
  ...
  SparseNode* p = NULL;
  p = SparseArray_insert(p, array -> index, array -> value);
  if (array -> left != NULL)
    p -> left = SparseArray_copy(array -> left);
  if (array -> right != NULL)
    p -> right = SparseArray_copy(array -> right);
  return p;
}
...
SparseNode * SparseArray_merge(SparseNode * array_1, SparseNode * array_2) {
  ...
  return SparseArray_mergeHelper(p, array_2);
}
